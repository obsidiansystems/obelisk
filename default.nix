    ...
    let configPath = base + "/config";
        projectOut = sys: (getReflexPlatform sys).project (args@{ nixpkgs, ... }:
          let mkProject = { android ? null #TODO: Better error when missing
                          ...
                          , packages ? {}
                          , closureCompilerOptimizationLevel ? "ADVANCED"
                          , overrides ? _: _: {}
              ...
              in {
                ...
                passthru = { inherit android ios packages overrides tools shellToolOverrides withHoogle staticFiles closureCompilerOptimizationLevel; };
              };
          in mkProject (projectDefinition args));
      serverOn = sys: config: serverExe (projectOut sys).ghc.backend (projectOut system).ghcjs.frontend (projectOut sys).passthru.staticFiles config (projectOut sys).passthru.closureCompilerOptimizationLevel;
