  ...
  compressedJs = frontend: optimizationLevel: externs: pkgs.runCommand "compressedJs" {} ''
    ...
    for f in **/all.js; do
      ...
      '' else ''
        ...
        '${pkgs.closurecompiler}/bin/closure-compiler' --error_format JSON ${if externs == null then "" else "--externs '${externs}'"} --externs '${mars + "/ghcjs.externs.js"}' -O '${optimizationLevel}' --jscomp_warning=checkVars --warning_level=QUIET --create_source_map="$dir/all.js.map" --source_map_format=V3 --source_map_location_mapping "$dir/all.unminified.js|/all.unminified.js" --js_output_file="$dir/all.js" "$dir/all.unminified.js"
