...
, config ? { }
, terms ? {
    # Accepted terms, conditions, and licenses
...
rec {
  ...
  project = base': projectDefinition:
    let
      projectOut = { system, enableLibraryProfiling ? profiling }:
        let reflexPlatformProject = (getReflexPlatform { inherit system enableLibraryProfiling; }).project; in reflexPlatformProject (args@{ nixpkgs, ... }:
          let
            ...
            mkProject =
              ...
              let
                allConfig = nixpkgs.lib.makeExtensible (self: {
                  ...
                  processedStatic = processAssets {
                    ...
                    exe =
                      if lib.attrByPath [ "userSettings" "__deprecated" "useObeliskAssetManifestGenerate" ] false self
              ...
              allConfig;
          in
          (mkProject (projectDefinition args)).projectConfig);
