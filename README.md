...
To run the application locally over https, update the protocol in `config/common/route` to `https`, and then use `ob run` as normal. Obelisk will automatically generate a self-signed certificate for running https. The certificate will be valid for 1 year, at which point a new certificate can be generated by removing `certificate.pem` from `~/.config/obelisk`.

Note: The browser will issue a warning about using an invalid certificate on localhost. On Chrome, you can either go to `chrome://flags/#allow-insecure-localhost` to enable invalid certificates for localhost, or click through the warning to add an exception for Obelisk's self-signed certificate.
