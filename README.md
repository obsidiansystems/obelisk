...
To run your app locally over https, update the protocol in `config/common/route` to `https`, and then use `ob run` as normal. Obelisk will generate a self-signed certificate for running https which will be valid for one year, at which point a new certificate can be generated by removing `certificate.pem` from `~/.config/obelisk` and restarting the app.
