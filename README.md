...
To try out the **experimental** [`ghcide`](https://github.com/digital-asset/ghcide) support:
  ...
  * Start your editor in a way that it can access the appropriate `ghcide`:
      ...
      * If you can't easily start your editor in a way that it inherits your shell, you may need customize how your `ghcide` plugin finds `ghcide` or create a custom `ghcide` replacement script that you install globally. For example, for Visual Studio Code on macOS adding a script called `ghcide` to your `PATH` like the one below will casue `ghcide` from each project's `ob shell` to be used:
          ```shell
          #!/usr/bin/env bash
