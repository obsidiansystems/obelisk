...
* Feature: Add deploy command `ob deploy build`. This will build the derivation like `ob deploy push`, but won't actually deploy the build. Later invocations of `ob deploy push` will not need to rebuild the app.
