...
import Control.Category.Associative (Associative (disassociate), associate)
...
import Data.Tabulation (HasFields (Field, fieldLens, tabulateFieldsA))
...
instance Arbitrary (DMap XYField Identity) where
  arbitrary =
    ...
    where
      opt k v = oneof $ fmap pure [mempty, DMap.singleton k v]

data A = A deriving (Bounded, Enum, Eq, Ord, Show)
data B = B deriving (Bounded, Enum, Eq, Ord, Show)
data C = C1 | C2 deriving (Bounded, Enum, Eq, Ord, Show)
instance Universe A
instance Universe B
instance Universe C
...
ac :: Encoder' A C
ac = generalizeIdentity $ handleEncoder (const A) $ enumEncoder $ \A -> C1
...
mkRoundtripTestTree lbl f = testProperty lbl $ withMaxSuccess 1e3 $ \(a, x) -> withCheckedEncoder (f x) (`roundtripsProp` a)
...
withEncoders2 xs ys f = liftA2 g xs ys
  where
    g (n1, Ex e1) (n2, Ex e2) = f (n1 <> "," <> n2) e1 e2
...
atomicEncoders =
  let t n e = (n, Ex e)
  in [ t "addPathSegmentEncoder" addPathSegmentEncoder
...
exhaustive :: TestTree
exhaustive =
  let prop :: Cont (forall a b. (Eq a, Finite a) => TestName -> Encoder' a b -> TestTree)
      prop f = f $ \lbl e -> testProperty lbl $ withCheckedEncoder e $ flip all universeF . roundtripsProp
  in testGroup "Roundtrip" $ prop $ \t ->
...
overlaps :: TestTree
overlaps =
  let prop :: (forall x y. Either x y -> Bool) -> Cont (forall a b. TestName -> Encoder' a b -> TestTree)
      prop is f = f $ \n -> testProperty n . is . checkEncoder @(Either Text)
  in testGroup
       "Overlaps"
       [ testGroup "No false positives" $
           prop isRight $
             const
               []
  ...
  testGroup "Roundtrip" $
    fold
      ...
      , arity0 $ \t ->
          ...
          , let sampleSize = ceiling @Double @Int . sqrt . fromIntegral . length
