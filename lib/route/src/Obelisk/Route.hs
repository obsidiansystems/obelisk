...
instance MonadError Text parse => HasInitialObject (EncoderImpl parse) where
  type Initial (EncoderImpl parse) = Void
  initiate = EncoderImpl
    { _encoderImpl_encode = \case
    , _encoderImpl_decode = \_ -> throwError "initiate: can't decode anything"
    }

instance (Applicative check, MonadError Text parse) => HasInitialObject (Encoder check parse) where
  type Initial (Encoder check parse) = Initial (EncoderImpl parse)
  initiate = Encoder $ pure $ initiate
...
void1Encoder = initiate . viewEncoder (iso (foldSome (\case)) (\case))
