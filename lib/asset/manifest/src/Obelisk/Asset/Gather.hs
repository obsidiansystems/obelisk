...
gatherHashedPaths
  ...
  -- ^ A mapping of original paths to hashed paths (all relative to the root)
  ...
  where
    ...
    go subdir = do
      ...
      fmap mconcat $ forM subs $ \sub -> do
        ...
        if isFile
          then do
            !hashedRelativePath <- force <$> toHashedPath root relativePath
            pure $ Map.singleton relativePath hashedRelativePath
...
toHashedPath root relativePath = do
  ...
  let hashPrefix = T.unpack $ decodeUtf8Lenient $ LBS.toStrict $ toNixBase32 $ bytestringDigest $ sha256 contents
      (dir, filename) = splitFileName relativePath
      !hashedRelativePath = force $ normalise $ dir </> (hashPrefix <> "-" <> filename)
  pure hashedRelativePath
