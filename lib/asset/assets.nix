...
rec {
  ...
  chunkDerivations = n: l:
    let
      ...
      makeChunkDerivation = chunk': next:
        ...
        in
        builtins.seq (builtins.toPath next) (nixpkgs.stdenv.mkDerivation {
  ...
  readDirRecursive = dir:
    let
      d = filterAttrs (n: d: !(hasPrefix "." n)) (builtins.readDir dir);
      go = name:
        let
          path = dir + "/${name}";
          type = d.${name};
    ...
    in
    builtins.listToAttrs (map go (builtins.attrNames d));
  ...
  dirToPath = contents:
    let
      pairsToBashArray = xs: builtins.concatStringsSep " " (map (np: "[" + doubleQuoteString np.fst + "]=" + doubleQuoteString (toString np.snd)) xs);
  ...
  mkValidDrvName = { str, replacement ? "?" }:
    ...
    in
    if builtins.substring 0 1 newName == "." then "_" + newName else newName;
